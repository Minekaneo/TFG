...
void loop()
{
    ...
    //Foto button
        ...
        //Captures the pixels on the camera
        ...
        //Transforms bytes to shorts
        //for (int i = 0; i < 176; i++) {
        //    for (int j = 0; j < 144; j++) {
        //        intimage[i * 144 + j] = ((unsigned char)data[i * 144 * 2 + j * 2] << 8) | (unsigned char)data[i * 144 * 2 + j * 2 + 1];
        //    }
        //}
        ...
        if (modo != 4) {
            ...
            float myTarget[3] = {0};
            myTarget[modo-1] = 1.f; // button 1 -> {1,0,0};  button 2 -> {0,1,0};  button 3 -> {0,0,1}
